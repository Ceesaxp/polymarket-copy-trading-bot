...
impl RustClobClient {
    ...
    /// * `private_key` - Wallet private key (64-char hex)
    /// * `funder` - Optional funder address. If None, derives from private_key (recommended).
    ///              Only use Some() if you have a separate funder with delegation set up.
    pub fn new(host: &str, chain_id: u64, private_key: &str, funder: Option<&str>) -> Result<Self> {
        ...
        // Use provided funder or derive from wallet (recommended default)
        // If funder differs from wallet, use signature_type 1 (Poly proxy)
        let (funder_address, signature_type) = match funder {
            Some(f) => (f.to_string(), 1), // Proxy wallet: maker != signer
            None => (wallet_address_str.clone(), 0), // EOA: maker == signer
        };
        ...
        Ok(Self {
            ...
            signature_type,  // 0 = EOA wallet, 1 = Poly proxy (Magic), 2 = Gnosis Safe
    ...
    pub fn l1_headers(&self, nonce: u64) -> Result<HeaderMap> {
        ...
        headers.insert(
            "POLY_ADDRESS",
            HeaderValue::from_str(&self.wallet_address_str)?,
        );
        headers.insert("POLY_SIGNATURE", HeaderValue::from_str(&sig.to_string())?);
        headers.insert(
            "POLY_TIMESTAMP",
            HeaderValue::from_str(itoa_buf.format(timestamp))?,
        );
    ...
    pub fn post_order_fast(
        ...
        // Verbose logging when VERBOSE_ORDER_LOG is set
        let verbose = std::env::var("VERBOSE_ORDER_LOG").is_ok();
        if verbose {
            eprintln!("\n{}", "=".repeat(80));
            eprintln!("📤 ORDER SUBMISSION (verbose mode)");
            eprintln!("{}", "=".repeat(80));
            eprintln!("URL: {}", url);
            eprintln!("\n📋 Headers:");
            for (name, value) in headers.iter() {
                ...
                if name.as_str() == "POLY_SIGNATURE" {
                    eprintln!("   {}: {}...{} chars", name, &val_str[..20.min(val_str.len())], val_str.len());
                } else {
                    eprintln!("   {}: {}", name, val_str);
                }
            }
            eprintln!("\n📦 Request Body:");
            // Pretty print JSON if possible
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(&body) {
                eprintln!("{}", serde_json::to_string_pretty(&json).unwrap_or(body.clone()));
            } else {
                eprintln!("{}", body);
            }
            eprintln!("{}", "-".repeat(80));
        ...
        let resp = self.http.post(&url).headers(headers).body(body).send()?;
        ...
        if verbose {
            ...
            // Note: Can't log body here as it would consume the response
            // The caller should log the body after reading it
            eprintln!("{}", "=".repeat(80));
        }
    ...
    pub fn create_order(&mut self, args: OrderArgs) -> Result<SignedOrder> {
        ...
        if std::env::var("VERBOSE_ORDER_LOG").is_ok() {
            eprintln!("\n{}", "=".repeat(80));
            eprintln!("🔐 ORDER CREATION (verbose mode)");
            eprintln!("{}", "=".repeat(80));
            eprintln!("Chain ID:    {}", self.chain_id);
            eprintln!("Exchange:    {}", exchange);
            eprintln!("Neg Risk:    {}", neg_risk);
            eprintln!();
            eprintln!("📋 Order Data:");
            eprintln!("   maker:         {}", data.maker);
            eprintln!("   signer:        {}", data.signer);
            eprintln!("   taker:         {}", data.taker);
            eprintln!("   token_id:      {}...", &data.token_id[..20.min(data.token_id.len())]);
            eprintln!("   side:          {} (0=BUY, 1=SELL)", data.side);
            eprintln!("   maker_amount:  {} (raw units)", data.maker_amount);
            eprintln!("   taker_amount:  {} (raw units)", data.taker_amount);
            eprintln!("   nonce:         {}", data.nonce);
            eprintln!("   expiration:    {}", data.expiration);
            eprintln!("   signature_type: {}", data.signature_type);
            eprintln!("   salt:          {}", data.salt);
            eprintln!();
            eprintln!("🔍 Address Check:");
            if data.maker == data.signer {
                eprintln!("   ✅ maker == signer (self-signed order)");
            } else {
                eprintln!("   ⚠️  maker != signer (delegated signing)");
                eprintln!("      This requires maker to have approved signer as operator!");
            }
            eprintln!("{}", "-".repeat(80));
        }
        ...
        // Debug: show the exact TypedData JSON being signed
        if std::env::var("VERBOSE_ORDER_LOG").is_ok() {
            if let Ok(json) = serde_json::to_string_pretty(&typed) {
                eprintln!("\n📜 TypedData JSON (for signing):");
                eprintln!("{}", json);
            }
        }
        ...
        // Debug: show the signing hash
        if std::env::var("VERBOSE_ORDER_LOG").is_ok() {
            eprintln!("\n🔑 EIP-712 signing hash: 0x{}", hex::encode(digest.as_slice()));
        }
        let sig = self
            .wallet
            .sign_hash_sync(&digest)
    ...
    /// Returns the wallet address (derived from private key) - used as 'signer' in orders
    pub fn wallet_address(&self) -> &str {
        &self.wallet_address_str
    }

    /// Returns the funder address - used as 'maker' in orders
...
#[inline(always)]
fn round_down(x: f64, d: u32) -> f64 {
    let f = 10f64.powi(d as i32);
    (x * f).floor() / f
}
#[inline(always)]
fn round_normal(x: f64, d: u32) -> f64 {
    let f = 10f64.powi(d as i32);
    (x * f).round() / f
}
...
fn decimal_places_fast(x: f64) -> u32 {
    let abs_x = x.abs();
    if abs_x == 0.0 || abs_x.fract() < 1e-10 {
        return 0;
    }
    for p in 1..=10u32 {
...
fn to_token_decimals(x: f64) -> Result<u128> {
    ...
    let val = if decimal_places_fast(scaled) > 0 {
